 ...
 * description:
 */
public class NViewOffsetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {

    private NViewOffsetHelper mViewOffsetHelper;

    private int mTempTopBottomOffset = 0;
    private int mTempLeftRightOffset = 0;

    public NViewOffsetBehavior() {}

    public NViewOffsetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mViewOffsetHelper == null) {
            mViewOffsetHelper = new NViewOffsetHelper(child);
        }
